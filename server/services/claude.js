 ...
 * Returns null if CLAUDE_API_KEY is not configured.
 */
function getClient() {
...
async function categorizeExpense(description, amount) {
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 50,
      system: `You are a financial categorization assistant. Your job is to categorize expenses into exactly one of these categories: ${EXPENSE_CATEGORIES.join(', ')}. 
Respond with ONLY the category name, nothing else. No explanation, no punctuation, just the category.`,
      messages: [
        {
          role: 'user',
          content: `Categorize this expense: "${description}" for ₹${amount}`,
        },
      ],
    });
    ...
    console.error('Claude categorization error:', error.message);
    return mockCategorize(description);
  }
}

/**
 * Chat with Claude about the user's expense data.
...
async function chatWithData(userMessage, expenseData) {
  ...
  try {
    ...
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 1024,
      system: `You are a friendly and knowledgeable personal financial advisor AI assistant embedded in an expense tracking app. You have access to the user's financial data and should provide helpful, actionable insights.
...
async function generateMonthlyReport(monthlyData) {
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 2048,
      system: `You are a financial report generator. Create detailed, insightful monthly financial reports. 
...
Be specific, reference actual numbers, and make tips actionable. Only output JSON, nothing else.`,
      messages: [
        {
          role: 'user',
          content: `Generate a monthly financial report for this data:
...
${monthlyData.savingsProgress ? `Target: ₹${monthlyData.savingsProgress.target?.toFixed(2) || '0.00'}, Current: ₹${monthlyData.savingsProgress.current?.toFixed(2) || '0.00'}` : 'No savings goals set'}`,
    ...
    try {
      ...
      console.error('Failed to parse Claude report response as JSON:', parseError.message);
      // Return the text as a summary with defaults
      return {
        summary: text,
        highlights: [],
        concerns: [],
    ...
    console.error('Claude report generation error:', error.message);
    return mockMonthlyReport(monthlyData);
  }
}
...
/**
 * Format expense data into a readable string for the Claude prompt.
 */
...
/**
 * Mock categorization when API key is not available.
 */
...
/**
 * Mock chat response when API key is not available.
 */
function mockChatResponse(userMessage, expenseData) {
  ...
  // 3. Category specific query (e.g., "how much on food", "what about electronics")
...
*Note: I am running in local Mock Mode. To enable full generative AI, please add a valid Claude API key in the .env file!*`;
}

/**
 * Mock monthly report when API key is not available.
 */
